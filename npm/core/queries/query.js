...
var Query = function (_Root) {
  ...
  function Query(options) {
    ...
    if ('paginationWindow' in options) {
      ...
      if (options.paginationWindow !== paginationWindow) {
        _utils.logger.warn('paginationWindow value ' + paginationWindow + ' in Query constructor ' + ('excedes Query.MaxPageSize of ' + _this._getMaxPageSize()));
      }
    }

    _this.data = [];
    _this._initialPaginationWindow = _this.paginationWindow;
    _settings.client.on('all', _this._handleEvents, _this);
  ...
  _createClass(Query, [{
    ...
    value: function destroy() {
      ...
      _settings.client.off(null, null, this);
      _settings.client._removeQuery(this);
      this.data = null;
    ...
    value: function update() {
      ...
      if ('paginationWindow' in optionsBuilt && this.paginationWindow !== optionsBuilt.paginationWindow) {
        ...
        if (this.paginationWindow < optionsBuilt.paginationWindow) {
          _utils.logger.warn('paginationWindow value ' + optionsBuilt.paginationWindow + ' in Query.update() ' + ('increases size greater than Query.MaxPageSize of ' + this._getMaxPageSize()));
        }
        needsRefresh = true;
      }
      if ('model' in optionsBuilt && this.model !== optionsBuilt.model) {
        throw new Error(_layerError.ErrorDictionary.modelImmutable);
      }
    ...
    value: function _fixPredicate(inValue) {
      if (inValue) throw new Error(_layerError.ErrorDictionary.predicateNotSupported);
      return '';
    }
    ...
    value: function _reset() {
      ...
      var data = this.data;
      this.data = [];
      _settings.client._checkAndPurgeCache(data);
      this.isFiring = false;
    ...
    value: function _run() {
      ...
      if (pageSize < 0) {
        ...
        _settings.client._checkAndPurgeCache(removedData);
        this.pagedToEnd = false;
    ...
    value: function _appendResults(results, fromDb) {
      ...
      results.data.forEach(function (item) {
        if (!(item instanceof _root2.default)) _settings.client._createObject(item);
      });
      ...
      newResults.forEach(function (itemIn) {
        var item = _settings.client.getObject(itemIn.id);
        if (item && (!_this3.filter || _this3.filter(item))) {
          _this3._appendResultsSplice(item);
    ...
    key: '_getInsertIndex',
    value: function _getInsertIndex(item, data) {
      return data.length;
    }
    ...
    value: function _getInstance(item) {
      if (item instanceof _root2.default) return item;
      return _settings.client.getObject(item.id);
    }
    ...
    value: function _handleAddEvent(name, evt) {
      ...
      var list = evt[name].filter(function (obj) {
        ...
        return !_this4.filter || _this4.filter(item);
      });
    ...
    value: function _triggerChange(evt) {
      if (this.isDestroyed || _settings.client._inCleanup) return;
 ...
 * @event error
 */
'error'].concat(_root2.default._supportedEvents);

_root2.default.initClass.apply(Query, [Query, 'Query', _namespace2.default]);
