...
module.exports = {
  ...
  methods: {
    ...
    animatedScrollTo: function animatedScrollTo(position) {
      ...
      var cancel = this.properties.cancelAnimatedScroll = (0, _uiUtils.animatedScrollTo)(this, position, animateSpeed, function () {
    ...
    _generateFragmentItem: function _generateFragmentItem(item, fragment) {
      ...
      var itemInstance = item instanceof _core2.default.Root ? item : _settings.client.getObject(item.id);
      if (itemInstance) {
      ...
      widgets.forEach(function (widget, index) {
        if (_this6.properties.onRenderListItem) {
          try {
            ...
            _utils.logger.error('Error in onRenderListItem for ' + widget.item.id + '; ' + err);
          }
        }
      }, this);
    },
    ...
    _runFilter: function _runFilter() {
      if (!this.filter) {
        this.querySelectorAllArray('.layer-item-filtered').forEach(function (item) {
          return item.classList.remove('layer-item-filtered');
        });
      } else {
        for (var i = 0; i < this.childNodes.length; i++) {
          var listItem = this.childNodes[i];
          if (listItem.item instanceof _core2.default.Root) {
