            ...
            // Message has been acknowledged by the server, but has not been delivered to anyone
              ...
              // Message is a Channel Message where read/delivery is not tracked
                ...
                // One-on-One Conversations
                  ...
                  // Group Conversations
                  else if (message.readStatus === _constants2.default.RECIPIENT_STATE.NONE) {
                      var count = Object.keys(message.recipientStatus).filter(function (id) {
                        return message.recipientStatus[id] === _constants2.default.RECEIPT_STATE.DELIVERED || message.recipientStatus[id] === _constants2.default.RECEIPT_STATE.READ;
                      }).length - 1;
                      html = this.deliveredGroupTemplate.replace(/(\$\{.*?\})/g, function (match) {
                        return count;
                      });
                    } else {
                      var _count = Object.keys(message.recipientStatus).filter(function (id) {
                        return message.recipientStatus[id] === _constants2.default.RECEIPT_STATE.READ;
                      }).length - 1;
                      html = this.readGroupTemplate.replace(/(\$\{.*?\})/g, function (match) {
                        return _count;
                      });
                    }
        this.innerHTML = html;
