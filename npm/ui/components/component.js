...
var logger = _utils2.default.logger;
/*
...
function setupMixin(classDef, mixin) {
  ...
  propNames.forEach(function (name) {
    ...
    if (!classDef.properties[name]) {
      ...
      classDef.properties[name] = _utils2.default.shallowClone(mixin.properties[name]);
    } else {
...
function finalizeMixinMerge(classDef) {
  ...
  propNames.forEach(function (name) {
    if (classDef['__' + name]) {
      ...
      var setters = _utils2.default.sortBy(classDef['__' + name].filter(function (def) {
        return def.set;
      }), function (setter) {
        switch (setter.mode) {
  ...
  methodNames.forEach(function (methodName) {
    ...
    var methodList = [].concat(_toConsumableArray(methodDef.methodsBefore), _toConsumableArray(methodDef.methodsMiddle), _toConsumableArray(methodDef.methodsAfter), _toConsumableArray(methodDef.methodLast));
    if (methodDef.lock) methodList = [methodDef.lock].concat(_toConsumableArray(methodDef.methodLast));
...
function getTheMethod(classDef, conditionals, methods) {
  return function runMixinMethods() {
    ...
    methods.forEach(function runMixinMethodsIterator(method) {
      var resultTmp = method.apply(this, args);
...
function setupEvent(classDef, eventName) {
  var camelEventName = _utils2.default.camelCase(eventName.replace(/^layer-/, ''));
...
function getPropArray(classDef) {
  ...
  return Object.keys(classDef.properties).map(function (propertyName) {
    return {
      ...
      attributeName: _utils2.default.hyphenate(propertyName),
    ...
    if (oldValue !== value || this.properties._internalState.inPropInit.indexOf(name) !== -1) {
      ...
      if (propDef.propagateToChildren) {
        Object.keys(this.nodes).forEach(function (nodeName) {
          _this.nodes[nodeName][name] = value;
        });
        if (this._isList) {
      ...
      if (listeners) listeners.forEach(function (fn) {
        return fn(value, wasInit ? null : oldValue, _this);
      });
    }
  };
...
function registerComponent(tagName, classDef) {
  if (!_componentServices.ComponentsHash[tagName]) _componentServices.ComponentsHash[tagName] = {};
  _componentServices.ComponentsHash[tagName].def = classDef;

  if (classDef.template) {
    _componentServices.ComponentsHash[tagName].template = classDef.template;
  }
  delete classDef.template; // deletes templates that are empty and fail the above test

  if (classDef.style) {
    _componentServices.ComponentsHash[tagName].style = classDef.style;
...
function _registerComponent(tagName) {
  var classDef = _componentServices.ComponentsHash[tagName].def;
  var template = _componentServices.ComponentsHash[tagName].template;


  if (template) {
    if (typeof template === 'string') {
      (0, _componentServices.buildAndRegisterTemplate)(tagName, template);
    } else if (template.getAttribute('layer-template-registered') !== 'true') {
      (0, _componentServices.registerTemplate)(tagName, template);
    }
  }
  ...
  if (_settings2.default._mixins[tagName]) {
    classDef.mixins = classDef.mixins.concat(_settings2.default._mixins[tagName]);
  }
  ...
  classDef.createdCallback = {
    value: function createdCallback() {
      ...
      if (!_componentServices.ComponentsHash[tagName]) return;
  ...
  classDef._setupListeners = {
    value: function _setupListeners() {
      ...
      this._listeners.forEach(function (eventName) {
        document.body.addEventListener(eventName, _this3._handleListenerEvent.bind(_this3, '__listener-' + eventName));
      });
    }
  };
  ...
  classDef._onAfterCreate = {
    value: function _onAfterCreate() {
      ...
      this.properties._internalState.inPropInit = _componentServices.ComponentsHash[tagName].properties.map(function (propDef) {
      ...
      props.forEach(function (prop) {
        ...
        if (value instanceof _core2.default.Root && value.isDestroyed) return;
        ...
        var isValueUnset = value === undefined || value === null;
        if (!isValueUnset) {
        ...
        if ((isValueUnset || value === prop.value) && _this4.parentComponent) {
          // Get the parentComponent's Property Definition
          var parentComponentProps = _componentServices.ComponentsHash[_this4.parentComponent.tagName.toLocaleLowerCase()].properties;
          var parentComponentPropDef = parentComponentProps.filter(function (p) {
            return p.propertyName === prop.propertyName;
          })[0];
  ...
  classDef.detachedCallback = {
    value: function detachedCallback() {
      ...
      var isMainComponent = this.mainComponent === this;
      this.onDetach();
      ...
      setTimeout(function () {
        if (_this6.properties._internalState.onDestroyCalled) return;
        if (document.body.contains(_this6) || document.head.contains(_this6)) return;
        if (_this6.trigger('layer-widget-destroyed', { target: _this6 })) {
          if (!isMainComponent || !document.body) {
            _this6.onDestroy();
          } else if (_this6.trigger.apply(document.body, ['layer-widget-destroyed', { target: _this6 }])) {
            _this6.onDestroy();
          }
        }
      }, _settings2.default.destroyAfterDetachDelay);
    }
  };
  ...
  classDef.attributeChangedCallback = {
    value: function attributeChangedCallback(name, oldValue, newValue) {
      ...
      this[_utils2.default.camelCase(name)] = newValue;
    }
  };
  ...
  _componentServices.ComponentsHash[tagName].classDef = document.registerElement(tagName, {
   ...
   * @static
   */
  _componentServices.ComponentsHash[tagName].properties = props;
}
  ...
  cssClassList: {
    set: function set(newValue, oldValue) {
      ...
      if (newValue) {
        ...
        newValue.forEach(function (cssClass) {
          return _this7.classList.add(cssClass);
        });
      } else {
      ...
      if (oldValue) {
        oldValue.forEach(function (cssClass) {
          if (!newValue || newValue.indexOf(cssClass) === -1) _this7.classList.remove(cssClass);
        });
      }
    }
  },
...
var standardClassMethods = {
  ...
  onReplaceableContentAdded: function onReplaceableContentAdded(name, node) {
    ...
    this._findNodesWithin(node, function (currentNode) {
      ...
      if (!currentNode.id) currentNode.id = _utils2.default.generateUUID();
      var layerId = currentNode.getAttribute('layer-id');
      var camelName = _utils2.default.camelCase(currentNode.tagName.toLowerCase().replace(/^layer-/, ''));
  ...
  _findNodesWithin: function _findNodesWithin(node, callback) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      if (innerNode instanceof HTMLElement) {
        var isLUIComponent = Boolean(_componentServices.ComponentsHash[innerNode.tagName.toLowerCase()]);
        var result = callback(innerNode, isLUIComponent);
        if (result) return result;

        // If its not a custom webcomponent with children that it manages and owns, iterate on it
        if (!isLUIComponent) {
          var innerResult = this._findNodesWithin(innerNode, callback);
  ...
  getTemplate: function getTemplate() {
    ...
    if (_componentServices.ComponentsHash[tagName].style) {
      var styleNode = document.createElement('style');
      styleNode.id = 'style-' + this.tagName.toLowerCase();
      styleNode.innerHTML = _componentServices.ComponentsHash[tagName].style;
      document.getElementsByTagName('head')[0].appendChild(styleNode);
      _componentServices.ComponentsHash[tagName].style = ''; // insure it doesn't get added to head a second time
    }
    return _componentServices.ComponentsHash[tagName].template;
  },
  ...
  createElement: function createElement(tagName, properties) {
    ...
    var ignore = ['parentNode', 'name', 'classList', 'noCreate'];

    Object.keys(properties).forEach(function (propName) {
      if (ignore.indexOf(propName) === -1) node[propName] = properties[propName];
    });
    if (properties.classList) properties.classList.forEach(function (className) {
  ...
  destroy: function destroy() {
    ...
    Object.keys(this.nodes || {}).forEach(function (name) {
      if (_this11.nodes[name] && _this11.nodes[name].destroy) _this11.nodes[name].destroy();
    });
    this.onDestroy();
  }
};
...
function unregisterComponent(tagName) {
  delete _componentServices.ComponentsHash[tagName];
}
...
function _registerAll() {
  if (!registerAllCalled) {
    ...
    Object.keys(_componentServices.ComponentsHash).filter(function (tagName) {
      return typeof _componentServices.ComponentsHash[tagName] !== 'function';
