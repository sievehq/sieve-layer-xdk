...
require('./layer-compose-bar');

require('./layer-typing-indicator');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

(0, _component.registerComponent)('layer-conversation-view', {
  mixins: [_hasQuery2.default, _focusOnKeydown2.default, _fileDropTarget2.default, _throttler2.default],
  template: '<layer-message-list layer-id=\'list\'></layer-message-list><layer-typing-indicator layer-id=\'typingIndicators\'></layer-typing-indicator><layer-compose-bar layer-id=\'composer\'></layer-compose-bar>',
  style: 'layer-conversation-view {\nposition: relative;\ndisplay: flex;\nflex-direction: column;\noutline: none;\n}\nlayer-message-list {\nflex-grow: 1;\nheight: 100px;\n}\nlayer-compose-bar {\nmin-height: 30px;\n}',
  ...
  properties: {
        ...
        if (_settings.client) {
          if (this.conversationId) {
            if (_settings.client.isReady && !_settings.client.isDestroyed) {
              this.conversation = _settings.client.getObject(this.conversationId, true);
            } else {
              _settings.client.once('ready', function () {
                if (_this.conversationId) _this.conversation = _settings.client.getObject(_this.conversationId, true);
              }, this);
            }
          } else {
    ...
    conversation: {
      set: function set(value) {
        if (value && !(value instanceof _core2.default.Conversation || value instanceof _core2.default.Channel)) {
          this.properties.conversation = null;
        }
        if (_settings.client) this._setupConversation();
      }
    },
    ...
    hasGeneratedQuery: {
      type: Boolean,
      set: function set(value) {
        if (value && this.conversation && _settings.client) this._setupQuery();
      }
    },
    ...
    autoFocusConversation: {
      value: _constants4.default.FOCUS.DESKTOP_ONLY
    },
      ...
      value: function getMenuItems(message) {
        return [{
          ...
          method: function method() {
            message.delete(_constants2.default.DELETION_MODE.ALL);
          }
        }];
      },
      set: function set(value) {
     ...
     */
    _queryModel: {
      value: _core2.default.Query.Message
    },
  ...
  methods: {
    ...
    _adjustForSoftKeyboard: function _adjustForSoftKeyboard() {
      var _this2 = this;

      setTimeout(function () {
        if (_this2.nodes.list.properties.stuckToBottom) {
          _this2.nodes.list.scrollToBottom(200);
        }
      }, 250);
    },
    ...
    _setupConversation: function _setupConversation() {
      ...
      if (_settings.client && !_settings.client.isReady) {
        _settings.client.once('ready', this._setupConversation, this);
        return;
       ...
       */
      this.trigger('layer-conversation-panel-change', { conversation: conversation });
    ...
    shouldAutoFocusConversation: function shouldAutoFocusConversation(_ref) {
      var maxTouchPoints = _ref.maxTouchPoints;

      switch (this.autoFocusConversation) {
        case _constants4.default.FOCUS.ALWAYS:
          return true;
        case _constants4.default.FOCUS.DESKTOP_ONLY:
          if (maxTouchPoints !== undefined && maxTouchPoints > 0) return false;
          return !_utils2.default.isMobile;
        case _constants4.default.FOCUS.NEVER:
          return false;
      }
      return false;
    },
     ...
     * @private
     */
    _setupQuery: function _setupQuery() {
      var conversation = this.properties.conversation;
      if (this.hasGeneratedQuery) {
        ...
        } else if (conversation instanceof _core2.default.Conversation) {
          this.query.update({
            predicate: 'conversation.id = "' + conversation.id + '"'
          });
        } else if (conversation instanceof _core2.default.Channel) {
          this.query.update({
