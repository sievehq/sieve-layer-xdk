...
require('./layer-choice-label-message-view');

require('./layer-choice-message-model');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

(0, _component.registerComponent)('layer-choice-message-view', {
  mixins: [_messageViewMixin2.default],
  template: '<div layer-id=\'label\' class=\'layer-choice-message-view-label\'></div><div layer-id=\'choices\' class=\'layer-choice-message-view-choices\'></div>',
  style: 'layer-choice-message-view .layer-choice-message-view-choices {\ndisplay: flex;\nflex-direction: column;\n}',
  properties: {
  ...
  methods: {
    ...
    onRerender: function onRerender() {
      ...
      this.model.choices.forEach(function (choice, index) {
        var button = _this2.nodes.choices.childNodes[index];
        button.text = _this2.model.getText(index);
        button.tooltip = _this2.model.getTooltip(index);
        button.selected = _this2.model.isSelectedIndex(index);
        button.disabled = !_this2.model.isSelectionEnabledFor(index);
      });
    },
    ...
    runAction: function runAction(_ref) {
      ...
      if (event === 'layer-choice-select') {
        ...
        var rootModel = _settings.client.getMessageTypeModel(rootPart.id);
